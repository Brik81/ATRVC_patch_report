...
\begin{document}
	...
	and 
	\begin{equation}
		W_p=\frac{c}{2f_r}\sqrt{\frac{2}{\epsilon_r+1}}
	\end{equation}
	where: 
	\begin{itemize}
		...
		\item $\epsilon_r$ is the dielectric constant of the substrate material, in our case is FR4
	...
	\begin{equation}
		\Delta L \simeq h \cdot 0.412 \frac{\left( \epsilon_\text{eff} + 0.3 \right) \left( \frac{W_p}{h} + 0.264 \right)}{\left( \epsilon_\text{eff} - 0.258 \right) \left( \frac{W_p}{h} + 0.8 \right)}
	\end{equation}
	
	and where $\epsilon_{eff}$ is the effective relative permittivity of a microstrip of width W:
	\begin{equation}
		\epsilon_\text{eff} \simeq \frac{\epsilon_r + 1}{2} + \frac{\epsilon_r - 1}{2} \frac{1}{\sqrt{1 + 12 \frac{h}{W_p}}}
	\end{equation}
	
	By imposing $L_\text{eff} = \frac{\lambda}{2}$ and knowing that $\lambda = \frac{c}{\sqrt{\epsilon_\text{eff}} f_r}$, it follows that:
	\begin{equation}
		L_p = \frac{c}{2 f_r \sqrt{\epsilon_\text{eff}}} - 2 \Delta L
	\end{equation}
...
\end{equation}
	where: 
	\begin{equation}
		G_1=\frac{W}{120 \lambda_0}(1-\frac{1}{24}(\frac{2 \pi h}{\lambda_0})^2)
	\end{equation}
	and 
	\begin{equation}
		\lambda_0=\frac{c}{f_r}
	...
	So, by using the the \eqref{eq7} and isolate the $x_0$ term ,we are able to find the value of the feeding line slot in order to have the impedance of 50 $\Omega$, having a final result similar to the image below. 
	\begin{figure}[h]
		\centering
